...
class SmallContactForm extends ComponentBase
{
  ...
  public function onFormSend(){
    ...
    if($validator->invalid() or !empty($errors)){
      ...
      $vars = ['fullname' => $this->post['full_name'],
      'position' => $this->post['position'],
      'mobile' => $this->post['mobile'],
      'email' => $this->post['email'],
      'companies_name' => $this->post['companies_name'],
      'address' => $this->post['address'],
      'messages' => $this->post['message'],
    ];
    $subject = "Name: " . " " .$this->post['full_name']. " " . " - ". " " .$this->post['companies_name'];
    Mail::send('janvince.smallcontactform::mail.message', $vars, function($message) use ($subject) {
        $message->to('info@devsamurai.com', 'Admin Person');
